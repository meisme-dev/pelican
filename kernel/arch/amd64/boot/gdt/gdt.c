...
static void gdt_set_tss(_gdt_entry_t *entries, _tss_t *tss) {
  entries[0].base0 = sizeof(_tss_t) - 1;
...
void gdt_init() {
  page_descriptor_t *mem = vmm_alloc_mem((sizeof(_gdt_entry_t) * DESC_COUNT) + sizeof(_tss_t));
  _tss_t *tss = (void *)mem->base;
  memset(tss, 0, sizeof(_tss_t));
  _gdt_entry_t *gdt = (void *)(mem->base + sizeof(_tss_t));
  memset(gdt, 0, sizeof(_gdt_entry_t) * DESC_COUNT);
